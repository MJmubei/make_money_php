...
(function(){
	...
	function Main(cfg){
		...
		this.config = {
			...
			maxSize : cfg.maxSize || 21474836480,
	        ...
	        isNeedAJAX : cfg.isNeedAJAX || false,
	        AJAXQueryURL : cfg.isNeedAJAX ? cfg.AJAXQueryURL : '',
    		BaseStorageURL : cfg.BaseStorageURL ? cfg.BaseStorageURL : '',
		};
		Parent.apply(this, arguments);
	}
	...
	Main.prototype = {
		...
		onSelect : function(list) {
			fShowMessage("查询需要文件数量: " + list.length);
		},
		onFileCountExceed : function(selected, limit) {
			fShowMessage("文件总数:" + selected + ", 但是限制数量:" + limit, true);
		},
		onMaxSizeExceed : function(file) {
			fShowMessage("文件名称:" + file.name + " ;文件大小:" + file.size +" 执行文件限制大小:" + file.limitSize, true);
		},
		onExtNameMismatch: function(file) {
			fShowMessage("允许文件扩展名称有: [" + file.filters.toString() + "], 不允许此扩展文件 " + file.name, true);
		},
		onAddTask: function(file) {
			fShowMessage("Add to task << name: [" + file.name);
		},
		onCancel : function(file) {
			fShowMessage("取消的文件: " + file.name);
		},
		onStop : function() {
			fShowMessage("暂停!");
		},
		...
		onComplete : function(file) {
			fShowMessage("文件名称:" + file.name + ", 文件大小:" + file.size + " 上传 [成功]");
			var temp_msg = JSON.parse(file.msg);
			if(this.config.isNeedAJAX && this.config.AJAXQueryURL.length>1)
			{
				$.post(this.config.AJAXQueryURL,{ action: "bg_upload", url: temp_msg.filePath,diskid:temp_msg.diskid },function(result){
					result = JSON.parse(result);
					if(result.ret == 0)
					{
						fShowMessage("生成介质库数据成功！文件路径:"+ this.config.BaseStorageURL +  file.name);
					}
					else
					{
						fShowMessage("<font color='red'>生成介质库数据失败！文件路径:"+ this.config.BaseStorageURL+ file.name);
					}
			    });
			}
		},
		...
		onUploadError : function(status, msg) {
			fShowMessage("错误操作！状态:" + status + ",消息: " + msg, true);
		},
		disable : function() {
		...
		validateFile : function(uploader) {
			...
			if(!bStreaming && size > 21474836480){this.uploadError({status:100, statusText:"Flash最大只能上传20G的文件!"});return !1;}
