...
require_once(SELF_PATH.'alipay_refund_response_v2.class.php');

class nl_alipay_refund_v2 implements nl_alipay_factory
{
    // 支付宝退款相关接口数据
    ...
    public function refund()
    {
        ...
        if (!$genrate_res)
        {
            return nl_alipay_refund_response::error(nl_alipay_refund_response::FAILD_GENRATE_BIZ_CONTENT,
                                            $this->error_msg);
        }
        ...
        if (!$response)
        {
            return nl_alipay_refund_response::error(nl_alipay_refund_response::FAILD_VALIDATE_CLIENT_PARAMS,
                                            $this->error_msg);
        }

        return nl_alipay_refund_response::parse($this->request->getApiMethodName(), $response);
    }
